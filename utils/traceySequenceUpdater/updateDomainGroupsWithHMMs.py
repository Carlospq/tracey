# Syncs new HMM files under utils/hmmModels/ into the rest of the system:
#   1. adds any missing entries to the staff family menu
#      (apps/templates/menus/query_sequences_full.py)
#   2. creates the corresponding Domaingroups rows in the database
#   3. rebuilds utils/hmmModels/MOTIFS.hmmDb (via rebuildMotifsHmmDb.py)
#
# Run as a management command:
#   python manage.py UpdateDomainGroups
# or from the "Update Domaingroups" card in features.html.
#
# The core logic is exposed as update_domaingroups_with_hmms() so the management
# command (apps/management/commands/UpdateDomainGroups.py) can run it detached
# from the request that triggered it — see apps/home/views_admin.py.
	...
	for dgKeyName in domaingroups_dict:
		...
		if not Domaingroups.objects.filter(domaingroupname__iexact=dgKeyName):
			...
			dg = Domaingroups(domaingroupname=dgKeyName,
							  ...
							  mappingstring='')
			dg.save()
			_log(f'  [db] created Domaingroup "{dgKeyName}" (domain {domain.domainname}, length {dgLen})')
...
def _run():
	...
	if changed:
		...
		_log('query_sequences_full.py updated.')
		importlib.reload(sys.modules['apps.templates.menus.query_sequences_full'])
		menu = importlib.import_module('apps.templates.menus.query_sequences_full').menu
	else:
		_log('No new HMMs found.')

	_log('')
	_log('=== Update DB domaingroups ===')
	for folder, config in DOMAIN_CONFIG.items():
		domain_name, menu_path = config[0], config[1]
		try:
			subtree = get_nested(menu, menu_path)
		except KeyError:
			_log(f'  [!] menu path {menu_path} not found, skipping {folder}')
			continue
		_log(f'  Processing {domain_name} ...')
		updateDomainGroups(subtree, domain=domain_name, dgparent=domain_name)

	_log('')
	_log('=== Rebuild MOTIFS.hmmDb ===')
	buf = io.StringIO()
...
def _render_html(text, ok):
	body_class = 'ok' if ok else 'err'
	heading = 'Update Domaingroups — completed' if ok else 'Update Domaingroups — FAILED'
	return (
		'<html><head><meta charset="utf-8"><style>'
		'body{font-family:monospace;padding:24px;background:#f8f8f8;}'
		'h2{margin-bottom:12px;} .ok{color:#2a7a2a;} .err{color:#c0392b;}'
		'pre{white-space:pre-wrap;word-break:break-word;background:#fff;'
		'border:1px solid #ddd;padding:16px;border-radius:6px;}'
		'</style></head><body>'
		f'<h2 class="{body_class}">{heading}</h2>'
		f'<pre>{html.escape(text)}</pre>'
		'</body></html>'
	)


def _write_log(log_path, html_report):
	with open(log_path, 'w', encoding='utf-8') as f:
		f.write(html_report)
		f.write('\n' + DONE_MARKER + '\n')


def update_domaingroups_with_hmms(log_path=None):
	"""
	Run the three sync phases (menu sync → Domaingroups rows → MOTIFS.hmmDb rebuild),
	collecting a line-by-line report.

	Returns the plain-text report. If log_path is given, also writes an HTML report
	there terminated by DONE_MARKER so the polling view can detect completion — see
	apps/home/views_admin.py:read_update_domaingroups_results.
	"""
	_report.clear()
	ok = True
	try:
	...
	text = '\n'.join(_report)
	if log_path:
		_write_log(log_path, _render_html(text, ok))
	return text
